...
app.get('/exercises', function(page, model, params, next) {
  // This value is set on the server in the `createUserId` middleware
  var userId = model.get('_session.userId');
  ...
  // Create a mongo query that gets the current user's exercises
  var exercisesQuery = model.query('exercises', {userId: userId});

  // Get the inital data and subscribe to any updates
  model.subscribe(user, exercisesQuery, function(err) {
    if (err) return next(err);

    // Create references that can be used in templates or controller methods
    model.ref('_page.user', user);
    exercisesQuery.ref('_page.exercises');

    user.increment('visits');
    page.render('exercises');
  });
});
...
app.fn('exercise.add', function(e, el) {
  var newItem = this.model.del('_page.newExercise');
  if (!newItem) return;
  newItem.userId = this.model.get('_session.userId');
  this.model.add('exercises', newItem);
});

app.fn('exercise.remove', function(e) {
  var exercise = e.get(':exercise');
  this.model.del('exercises.' + exercise.id);
});
